...
export default {
  ...
  methods: {
    getList () {
      ...
      this.$request.post('/processInstance/Settlement/1/list', obj).then(res => {
        if (res.code === 0) {
    ...
    suer (item) {
      ...
      this.$router.push({ path: '/settlement-detail', query: { id: item.processInstanceId, type: 2, souce: 'edit' } })
    }
  }
}
</script>
