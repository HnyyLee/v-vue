  ...
  <div class="allot_box">
    <div>
      <div class="allot_box_title">
        ...
        <div class="search_left">
          <span>按部门筛选：</span>
          <a-select placeholder="请选择部门"  style="width: 220px;" allowClear  @change="departSelectChange">
            <a-select-option v-for="(sel, index) in departSelectArr" :key="index" :value="sel.orgCode" >{{sel.orgName}}</a-select-option>
          </a-select>
          <a-button class="dataSerachBtn" style="margin-left: 20px;" type="primary" @click="submit" v-if="RouterNameFlag[mythat.$route.name] || RouterNameFlag['orderList_search']">搜索</a-button>
...
import { myorderManage } from './js/orderManage.js'
...
export default {
  data () {
    return {
      ...
      departSelectArr: [], // 顶部的部门数组
      departSelectNumb: '', // 顶部的部门的值
  ...
  methods: {
    ...
    getDepartmentFn() {
      this.$request.get('/organizeDataAccess/findLaborCityCompanyDeptList').then(res => {
        if (res.code === 0) {
          this.departSelectArr = res.result
        }
      })
    },
    // 改变顶部部门下拉框
    departSelectChange(value) {
      this.departSelectNumb = value
      if (!value) {
        this.getDataFun(this.mypassDataA, this.mypassDataB, this.myscreenFilterVos)
      }
    },
    getListOrderFn() {
      this.$request.get('/projectWorkOrder/keyword/list?status=' + '4').then(res => {
        if (res.code === 0) {
          ...
          this.cascaderoptions.forEach(item => {
            ...
            } else if (item.value == 'codes') {
              res.result.codes.forEach((pro,index) => {
                let aobj = {}
                aobj.label = pro
                aobj.value = index
                item.children.push(aobj)
    ...
    getDataFun (orderKeyword, orderStatus, myscreenFilterVos = []) { // 获取列表数据
      ...
      if (this.searchKeyStatus) {
        ...
        if (this.searchKeyStatus == 'projects') {
          ...
          anum = 1
        } else if (this.searchKeyStatus == 'codes'){
          anum = 6
        }
      ...
      if (this.departSelectNumb) {
        obj.deptCode = this.departSelectNumb
      }
      ...
      this.tableList = []
      let url = '/projectWorkOrder/getPageWorkOrderByCityCompany'
