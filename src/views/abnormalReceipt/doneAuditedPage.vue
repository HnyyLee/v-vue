    ...
    <div class="content">
      ...
      :locale="{ emptyText: '暂无数据' }" >
        ...
        <template slot="action" slot-scope="scope, aitem">
          <a-button type="primary" size="small" style="margin-right: 10px;" @click="tongSure(aitem, 1)" v-if="RouterNameFlag[mythat.$route.name] || RouterNameFlag['doneAuditedPage_Agree']">通过</a-button>
          <a-button  size="small" @click="refshFn(aitem, 2)" v-if="RouterNameFlag[mythat.$route.name] || RouterNameFlag['doneAuditedPage_Refuse']">拒绝</a-button>
        </template>
      </a-table>
    </div>
    <!-- 分页 -->
...
export default {
  ...
  methods: {
    ...
    getAuditedList() {
      ...
      this.$request.post('/processInstance/UnderLine/0/list', obj).then(res => {
        if (res.code === 0) {
    ...
    // 点击通过
    tongSure(scope, aitem) {
      this.shenpiVisible = true
      this.processInstanceId = scope.processInstanceId
      this.myisFlag = aitem
      this.ModalText = '是否通过当前异常收款审批'
    },
    // 点击拒绝
    refshFn(scope, aitem) {
      this.shenpiVisible = true
