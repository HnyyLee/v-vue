...
<template>
  <div class="quotedetails">
    <div class="contentDom">
      <!-- 第一步 -->
      <h3 class="top_h3">万科物业装修服务中心主材变更单</h3>
      ...
      <!-- 基础施工减少项目 -->
      <h3 class="center_Title">主材减少项目</h3>
      <a-table :columns="materColumns" :dataSource="materAddData" bordered :pagination="false">
      </a-table>

      <!-- 基础施工增加项目 -->
      <h3 class="center_Title">主材增加项目</h3>
      <a-table :columns="materColumns" :dataSource="materRedData" bordered :pagination="false">
      </a-table>
      <!-- 综合计费 -->
  ...
  methods: {
    ...
    showPrinterData() {
      ...
      this.$request.post('/workOrderAddDrop/findNewIODDetailById', sdata).then(res => {
        if (res.code === 0) {
          ...
          // 第一个表格减项的赋值
          this.materAddData = res.result.key2
          if (this.materAddData) {
            if (this.materAddData.length !== 0) {
              this.materAddData.push({
                num: 0,
                price: 0
              })
              this.materAddData.forEach((item, index) => {
                item.key = index
          ...
          // 第二个表格增项的赋值
          this.materRedData = res.result.key1
          if (this.materRedData) {
            if (this.materRedData.length !== 0) {
              this.materRedData.push({
                num: 0,
                price: 0
              })
              this.materRedData.forEach((item, index) => {
                item.key = index
                item.itemNum = item.num
    ...
    anumber(row) {
      if (row.isAddRedFlag == 1) {
        return this.materAddData.length - 1
      } else {
        return this.materRedData.length - 1
      }
    },
