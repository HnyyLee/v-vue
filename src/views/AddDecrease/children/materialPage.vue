<template>
  <div class="materialPage">
    <div class="alteration_h1">主材增加项</div>
    <a-button type='primary' class="addmain_btn" @click="alteratAddFn" v-if="ExamineFlag">添加SKU</a-button>
    <div class="table_Dom">
      <a-table :columns="materialListColumns" :dataSource="technoDataListOne" :rowKey="(record, index) => index" bordered :pagination='true' :locale="{ emptyText: '暂无数据' }">
        ...
        <template slot="spaceDes" slot-scope="text, record">
          <a-input :disabled='!ExamineFlag' placeholder="请输入空间" v-model="record.spaceDes" @change="spaceChangeOne(record.id, $event, record)"  min="0" />
        </template>
      </a-table>
      <div class="hold_Dom">
    ...
    <!-- 主材SKU弹框 -->
    <common-poplur ref="technoCommonPoplurOne" :dialogColums="materialAddColumns" :dialogData="technoPoplurListOne" textMapTitle="添加主材" @sumbitPoplur="technoSubmitReplaceOne" :searchData="materialSearchDataOne" :rowType='1' @topSearchThink="topSearchThinkOne" @cannalThink="cannalThinkOne">
    </common-poplur>


    <div class="alteration_h1">主材减少项</div>
    <a-button type='primary' class="addmain_btn" @click="alteratRedFn" v-if="ExamineFlag">添加SKU</a-button>
    <div class="table_Dom">
      <a-table :columns="materialListColumns" :dataSource="technoDataListTwo"  bordered :pagination='true' :locale="{ emptyText: '暂无数据' }">
        <template slot="itemNum" slot-scope="text, record">
        ...
        <template slot="spaceDes" slot-scope="text, record">
          <a-input :disabled='!ExamineFlag' placeholder="请输入空间" v-model="record.spaceDes" @change="spaceChangeTwo(record.id, $event, record)"  min="0" />
        </template>
      </a-table>
      <div class="hold_Dom">
    ...
    <!-- 主材SKU弹框 -->
    <common-poplur ref="technoCommonPoplurTwo" :dialogColums="materialAddColumns" :dialogData="technoPoplurListTwo" textMapTitle="添加主材" @sumbitPoplur="technoSubmitReplaceTwo" :searchData="materialSearchDataTwo" :rowType='1' @topSearchThink="topSearchThinkTwo" @cannalThink="cannalThinkTwo">
    </common-poplur>
    ...
    <div class="lastTable_Dom" v-if="showHoldLastTable">
      ...
      <a-table :columns="materTableColumns" :dataSource="lastTableData"  bordered :pagination='true' :locale="{ emptyText: '暂无数据' }">
      </a-table>
    </div>
...
import { materialListColumns, materialAddColumns, materialSearchDataOne, materialSearchDataTwo, materTableColumns } from '../js/colums'
import commonPoplur from '@/views/OfferManagement/children/commonPoplur/commonPoplur'
import { materialOne } from '../js/materialOne'
import { materialTwo } from '../js/materialTwo'
...
export default {
  data () {
    return {
      ...
      materialSearchDataOne, // 弹框的搜索数据
      allAddMountOne: 0, // 增加项的合计
      technoDataListTwo: [],
      technoPoplurListTwo: [], // table表格的数据
      materialSearchDataTwo, // 弹框的搜索数据
  ...
  mixins: [materialOne, materialTwo],
  components: {
    commonPoplur
  },
  methods: {
