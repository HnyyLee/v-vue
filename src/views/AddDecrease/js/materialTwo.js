  ...
  methods: {
    ...
    spliceRangAllMountTwo () {
      ...
      this.technoDataListTwo.forEach(item => {
        ...
        if (item.mydisprice) {
          item.amount = (Number(item.itemNum) * Number(item.mydisprice)).toFixed(2)
        } else {
    ...
    itemNumChangeTwo(adata, ev, aobj) {
      ...
      this.technoDataListTwo.forEach(icen => {
        if (icen.id == adata) {
          ...
          if (icen.mydisprice) {
            icen.amount = Number(myValue) * Number(icen.mydisprice)
          } else {
