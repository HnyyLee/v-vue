<template>
  <div class="construc">
    <!-- <div class="construc_header_menu">
      <span class="span_bgcolor">待结算工地</span>
      <span @click="goto">已结算工地</span>
    </div> -->
    <div class="construc_header">
      <a-input style="width: 30%" v-model="keyword" placeholder="搜索客户姓名、电话、房屋地址房号等" />
      <a-button style="margin-left: 10px;" @click="getDataList" type="primary" v-if="RouterNameFlag[mythat.$route.name] || RouterNameFlag['construction_search']" >查询</a-button>
    </div>
...
export default {
  ...
  methods: {
    go (e) { // 转跳
      ...
      this.$router.push({ path: '/construction-detail', query: { sourch: 'wjs', id: e.code } })
    },
    getDataList () { // 获取数据
      let obj = {
        pageNum: this.current, // 页数
        pageSize: this.pageSize, // 条数
        settlementStatus: 1, // 带结算
    ...
    // goto () {
    //   this.$router.push('/settlement-site')
    // }
  }
}
</script>
