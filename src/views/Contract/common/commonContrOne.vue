  ...
  <div class="commonContrOne">
    <table border="1" class="smallcomm">
      ...
      <tr>
        <th>施工内容</th>
        <td>
          <a-form-item class="patternDom">
            <a-input type="number" v-decorator="verify.room" :maxlength="2" />
          </a-form-item>
          <span class="spanText">室</span>
          <a-form-item class="patternDom">
            <a-input type="number" v-decorator="verify.office" :maxlength="2" />
          </a-form-item>
          <span class="spanText">厅</span>
          <a-form-item class="patternDom">
            <a-input type="number" v-decorator="verify.kitchen" :maxlength="2" />
          </a-form-item>
          <span class="spanText">厨</span>
          <a-form-item class="patternDom">
            <a-input type="number" v-decorator="verify.wei" :maxlength="2" />
          </a-form-item>
          <span class="spanText">卫</span>
          <a-form-item class="patternDom">
            <a-input type="number" v-decorator="verify.balcony" :maxlength="2" />
          </a-form-item>
          <span class="spanText spanTextsp">阳台</span>
        </td>
        <th>乙方收款账户开户名</th>
      ...
      <tr>
        ...
        <td>
          <a-form-item style="margin-bottom: 0;" >
            <a-date-picker @change="lineTimeStartFn" v-decorator="verify.disclosure" :disabledDate="lineStartDisableDate" />
          </a-form-item>
        </td>
        <th>二期款</th>
        <td>
      ...
      <tr>
        ...
        <td>
          <a-form-item style="margin-bottom: 0;" >
            <a-date-picker @change="lineTimeEndFn" v-decorator="verify.dismantling" :disabledDate="lineEndDisableDate" />
          </a-form-item>
        </td>
        <th>尾款</th>
        <td>
...
export default {
  ...
  methods: {
    ...
    // 打折交底和完成日期的change事件
    lineTimeStartFn(adata, atime) {
      if (atime) {
        this.lineStartTime = adata
      } else {
        this.lineStartTime = ''
      }
    },
    lineTimeEndFn(adata, atime) {
      if (atime) {
        this.lineEndTime = adata
      } else {
        this.lineEndTime = ''
      }
    },
    ...
    // 打折交底和完成日期的组件限制条件
    lineStartDisableDate(startValue) {
      const endValue = this.lineEndTime
      if (!startValue || !endValue) {
        return false;
      }
      return startValue.valueOf() > endValue.valueOf()
    },
    lineEndDisableDate (endValue) {
      const startValue = this.lineStartTime
